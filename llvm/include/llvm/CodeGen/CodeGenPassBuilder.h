...
#ifndef LLVM_CODEGEN_PASSBUILDER_H
#define LLVM_CODEGEN_PASSBUILDER_H
...
public:
  ...
  // For llc to build tiny pipeline from text.
  Expected<ModulePassManager> parseIRPipeline(StringRef) const;
  ...
  /// This method could be implemented by targets to provide target-specific
  /// analyses.
  void registerAnalyses(MachineFunctionAnalysisManager &MFAM) const {
    registerFunctionAnalyses(MFAM);
    registerModuleAnalyses(MFAM);
...
protected:
  ...
  class AddIRPass {
  public:
    AddIRPass(bool DebugPM, bool Check = true) : MPM(DebugPM), FPM(DebugPM) {
      if (Check)
  ...
  public:
    AddMachinePass(bool DebugPM, bool RequiresCodeGenSCCOrder,
        ...
        : PM(DebugPM, RequiresCodeGenSCCOrder),
          VerifyMachineCode(VerifyMachineCode) {}
    ...
    template <typename PassT> void operator()(PassT &&Pass) {
      ...
      if (VerifyMachineCode)
        PM.addPass(MachineVerifierPass());
    ...
    template <typename PassT> void insertPass(AnalysisKey *ID, PassT Pass) {
      AfterCallbacks.emplace_back(
          [this, ID, Pass = std::move(Pass)](AnalysisKey *PassID) {
            ...
            if (this->VerifyMachineCode)
              this->PM.addPass(MachineVerifierPass());
          });
    }
  ...
  private:
    ...
    // FIXME: Use PIC to insert IR/machine verification function. Need to pass
    //        down AnalysisManager in PassManager into PIC callbacks since
    //        machine verifier need analysis results to do verification.
    //        `-debugify-and-strip-all` could be handled similarly.
    bool VerifyMachineCode;
  };
  ...
  /// Target should override this to support adding individual target-specific
  /// pass from pass name (for debugging).
  Error addTargetPass(AddIRPass &, StringRef PassName) const {
    return make_error<StringError>("addTargetPass is not overridden",
                                   inconvertibleErrorCode());
  }
  Error addTargetPass(AddMachinePass &, StringRef PassName) const {
    return make_error<StringError>("addTargetPass is not overridden",
                                   inconvertibleErrorCode());
  }
...
private:
  ...
  void registerFunctionAnalyses(MachineFunctionAnalysisManager &) const;
  void registerModuleAnalyses(MachineFunctionAnalysisManager &) const;
...
CodeGenPassBuilder<Derived>::parseIRPipeline(StringRef PipelineText) const {
  AddIRPass addPass(Opt.DebugPM, false);

  SmallVector<StringRef, 4> Passes;
  PipelineText.split(Passes, ',');
...
#define LOOP_PASS(NAME, PASS_NAME, CONSTRUCTOR)                                \
  if (Name == NAME) {                                                          \
    addPass(createFunctionToLoopPassAdaptor(                                   \
        PASS_NAME CONSTRUCTOR, /*UseMemorySSA*/ true, Opt.DebugPM));           \
    continue;                                                                  \
  }
...
#include "MachinePassRegistry.def"
    ...
    if (auto Err = derived().addTargetPass(addPass, Name))
      return std::move(Err);
  }
...
CodeGenPassBuilder<Derived>::parseMIRPipeline(StringRef PipelineText) const {
...
void CodeGenPassBuilder<Derived>::registerFunctionAnalyses(
  ...
  MFAM.registerFunctionAnalysisPass(
      [this] { return registerAAAnalyses(this->Opt.UseCFLAA); });

  MFAM.registerFunctionAnalysisPass(
      [this] { return this->TM.getTargetIRAnalysis(); });

#define FUNCTION_ANALYSIS(NAME, PASS_NAME, CONSTRUCTOR)                        \
  MFAM.registerFunctionAnalysisPass([&] { return PASS_NAME CONSTRUCTOR; });
#include "MachinePassRegistry.def"
}
...
#define MODULE_ANALYSIS(NAME, PASS_NAME, CONSTRUCTOR)                          \
  MFAM.registerModuleAnalysisPass([&] { return PASS_NAME CONSTRUCTOR; });
#include "MachinePassRegistry.def"
}
...
#endif // LLVM_CODEGEN_PASSBUILDER_H
